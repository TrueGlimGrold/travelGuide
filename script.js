...
document.addEventListener('DOMContentLoaded', function() {
  fetch('countries.json')
    ...
    .then(data => {
      ...
      // Check if there is a selected country in local storage
      const storedCountry = localStorage.getItem('selectedCountry');
      if (storedCountry) {
        input.value = storedCountry;
        // Delay the dispatch of the input event
        setTimeout(() => {
      ...
      input.addEventListener('input', function() {
        ...
        localStorage.setItem('selectedCountry', input.value);
      });
    })
    .catch(error => {
